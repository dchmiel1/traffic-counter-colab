...
from trackers.smiletrack import matching
from trackers.smiletrack.gmc import GMC
from trackers.smiletrack.basetrack import BaseTrack, TrackState
from trackers.smiletrack.kalman_filter import KalmanFilter
...
class SMILEtrack(object):
    def __init__(self, **kwargs):
        ...
        self.track_high_thresh = kwargs.get("track_high_thresh", 0.3)
        self.track_low_thresh = kwargs.get("track_low_thresh", 0.1)
        self.new_track_thresh = kwargs.get("new_track_thresh", 0.4)

        self.match_thresh = kwargs.get("match_thresh", 0.7)

        self.buffer_size = int(kwargs.get("frame_rate", 30.0) / 30.0 * kwargs.get("track_buffer", 20))
        ...
        self.gmc = GMC(method=kwargs.get("cmc_method", "sparseOptFlow"), verbose=[kwargs.get("name", "exp"), kwargs.get("ablation", False)])
    ...
    def update(self, output_results, img):
        ...
        '''Extract embeddings '''
        if self.kwargs.get("with_reid", False):
        ...
        if not self.kwargs.get("mot20", False):
